"""A tiny stdlib HTTP helper shared by the transports (no third-party deps).

``default_request`` performs one HTTP request with ``urllib`` and returns
``(status, body)`` where ``body`` is the parsed JSON when the response is JSON,
otherwise the raw text. It is injectable so tests can run without a network.
"""
